...
AGENT_INSTRUCTIONS = """
너는 Microsoft Teams 미팅 중 실시간으로 감지된 IT 및 도메인(금융 또는 물류) 용어를 풀이해서 사용자의 이해를 돕는 '용어 설명 에이전트(Glossify Agent)'다.
...
[판단 규칙: 도메인]
- Finance, Logistics 중 가능하면 둘 중 하나로 명확히 분류.
- 둘 다 아니거나 애매하면 EnterpriseIT.
...
[예시 – 출력만 예시]
입력: term="AML", category="Skill", source_text="AML 시나리오 튜닝…"
출력: Finance. AML(자금세탁방지)은 금융기관이 의심거래를 탐지·보고하여 자금세탁을 차단하기 위한 규제·업무 체계를 말한다. 모델·룰 기반 탐지와 리스크 평가, 제재 대응 절차를 포함한다. 여기서는 거래 탐지 규칙(시나리오) 튜닝 맥락으로 보인다.
...
try:
    ...
    agent = project_client.agents.create_agent(
        ...
        name="glossify-term-explainer-v5",
